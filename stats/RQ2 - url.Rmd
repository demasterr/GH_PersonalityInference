...
dbs_res <- readdbs('selected_dev_parsed', 'selected_dev_parsed_url', transformations = transformations)

data_og <- dbs_res$db1
data_cmp <- dbs_res$db2

dfs <- list(data_og, data_cmp)
dfnames = c('original', 'URL')
rm(dbs_res)
```

In order to evaluate research question 2, we need to connect to the MySQL database first. From the database we fetch to dataframes, _data\_og_ (scores with all preprocessing steps enabled) and _data\_cmp_ (scores without at URL parsing). Any column indicating significance (signf) means a `p-value` below `0.05` is found.
...
```{r}
density_all(data_og, data_cmp, name2='urls')
```
...
latex_table(inspect_gt_non_zero, caption=paste("Difference in RMSE and MAE scores with the original dataset compared to the", dfnames[[2]],"dataset."), label=paste0("tab:",dfnames[[2]],"_gt_RMSE_MAE"), include_index=TRUE, fix_column=0, transpose=TRUE, df1=data_og[,diff_cols], df2=data_cmp[,diff_cols], dfnames=dfnames, gt=gt)
```
