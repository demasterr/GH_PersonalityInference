...
dbs_res <- readdbs('selected_dev_parsed', 'selected_dev_parsed_ip', transformations = transformations)

data_og <- dbs_res$db1
data_cmp <- dbs_res$db2

dfs <- list(data_og, data_cmp)
dfnames = c('original', 'IP')
rm(dbs_res)
```

In order to evaluate research question 2, we need to connect to the MySQL database first. From the database we fetch to dataframes, _data\_og_ (scores with all preprocessing steps enabled) and _data\_cmp_ (scores without at IP parsing). Any column indicating significance (signf) means a `p-value` below `0.05` is found.
...
```{r}
density_all(data_og, data_cmp, name2='IP')
```
...
repeated(shap, repeats, data_og, sample_size, p=0.001) # Check if the p-values are most of the time right.
repeated(shap, repeats, data_og, sample_size=sample_size, p=0.001) # Check if the p-values are most of the time right.
```
