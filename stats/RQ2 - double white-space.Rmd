...
dfnames = c('original', 'double whitespace')
rm(dbs_res)
```
...
We now reduce the dataframes to the columns that have some difference in them.
```{r}
# Reduce the columns to the columns that have an effect.
data_og <- data_og[1:13]
data_cmp <- data_cmp[1:13]
dfs <- list(data_og, data_cmp)
```
...
latex_table(func=inspect_AE_differences, 
            caption=paste("Differences in scores for PI with and without", dfnames[[2]], "parsing (not reporting traits without differences)."),
...
```{r}
density_all(data_og, data_cmp, name2='whitespace')
```
...
```{r}
wilc_df <- wilc(data_og, data_cmp, paired = TRUE, p = 0.05)
wilc_df
```
...
## T-test
```{r}
ttest_df <- ttest(data_og, data_cmp, p=0.05)
ttest_df
```
...
```{r}
latex_table(func=wilc, caption="Paired Wilcoxon signed-rank test between the original data and double white space parsing disabled.", label="tab:doublewhitespace_wilcox", df1=data_og[normal_cols], df2=data_cmp[normal_cols], p=0.05, paired=TRUE)
```

```{r}
latex_table(func=ttest_eff, caption="Paired t-test and Cohen's d effect sizes between the original data and double white space parsing disabled.", label="tab:doublewhitespace_ttest", df1=data_og[non_normal_cols], df2=data_cmp[non_normal_cols], p=0.05, paired=TRUE)
```
...
Lets only check the columns with a difference.
```{r}
inspection <- inspect_gt_non_zero(data_og, data_cmp, dfnames=dfnames, gt=gt)
inspection <- inspection[c(1,3,4,6)]
inspection
```

We see an improvement for PI C and E and slightly worsen for N.
