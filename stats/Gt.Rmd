...
```{r}
show_confint <- function(df, gt) {
  ae <- AE(df, gt)

  out <- data.frame(matrix(ncol=15,nrow=2), stringsAsFactors = FALSE)
  for (i in 2:15) {
    std <- sd(ae[,i])
    nfive <- std*0.95
    nnine <- std*0.99
...
correlations_df <- function(df, correlation_method) {
  ...
  for (trait_idx in 1:5) {
    ...
    yark_idx <- trait_idx + 6
    golb_idx <- trait_idx + 11
    colnames(out) <- colnames(data)[c(pi_idx, yark_idx, golb_idx)]
    row.names(out) <- colnames(out)
    
    out[1,2] <- cor.test(x=data[,pi_idx], data[,yark_idx], method=correlation_method)[["estimate"]]
    out[1,3] <- cor.test(x=data[,pi_idx], data[,golb_idx], method=correlation_method)[["estimate"]]
    out[2,3] <- cor.test(x=data[,yark_idx], data[,golb_idx], method=correlation_method)[["estimate"]]
    out[1,1] <- "-"
    out[2,2] <- "-"
    out[3,3] <- "-"
    out[2,1] <- out[1,2]
